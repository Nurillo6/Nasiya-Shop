...
import { Heading, MessageSkeleton, Text } from "../../../components";
...
const NotificationMessage = () => {
    ...
    const { data, isLoading } = useQuery<{ notifications: MessageType[], debtor: { name: string } }>({
        ...
        queryFn: () => instance.get("/notification", { params: { debtorId } }).then(res => res.data.data)
    })
    // Get All Messages
    ...
    const { mutate } = useMutation({
        mutationFn: (data: { message: string, debtorId: string | undefined }) => instance.post("/notification", data),
        onSuccess: () => {
    ...
    const { mutate: deleteMessage, isPending } = useMutation({
        mutationFn: () => instance.delete(`/notification/${debtorId}`),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['messages'] })
            navigate(-1)
        }
    ...
    return (
        <div className="containers">
            <div className="flex fixed z-50 top-0 pt-[30px] w-full bg-white max-w-[400px] items-center border-b-[1px] border-[#ECECEC] justify-between pb-[11px] mb-[28px]">
                ...
                <Heading tag="h2">{isLoading ? <div className="w-[200px] h-[25px]">
                    <SkeletonButton active className="!w-full !h-full !rounded-[20px] !overflow-hidden" />
                </div> :  data?.debtor?.name}</Heading>
            ...
            <div className="mt-[80px] pb-[80px] space-y-[20px]">
                {isLoading ? <MessageSkeleton/> : data?.notifications?.map((item, index) => (
                    <div key={item.id}>
